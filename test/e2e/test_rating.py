...
def test_create_rating_endpoint():
    """Test POST /api/rating endpoint and verify record persists in database."""
    token = get_auth_token()
    assert token is not None, "Failed to get auth token"

    headers = {"Authorization": f"Bearer {token}"}
    data = {
        "name": "e2e-test-rating",
        "description": "E2E test rating document",
    }

    response = requests.post(f"{BASE_URL}/api/rating", headers=headers, json=data)
    assert response.status_code == 201, _err(response, 201)

    response_data = response.json()
    assert "_id" in response_data, "Response missing '_id' key"
    assert response_data["name"] == "e2e-test-rating"
...
def test_get_ratings_with_name_filter():
    ...
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/rating?name=e2e", headers=headers)
    assert response.status_code == 200, _err(response, 200)
