...
These tests verify that Review endpoints work correctly by making
...
def test_create_review_endpoint():
    """Test POST /api/review endpoint and verify record persists in database."""
    ...
    data = {
        ...
        "description": "E2E test review document",
    }

    response = requests.post(f"{BASE_URL}/api/review", headers=headers, json=data)
    assert response.status_code == 201, _err(response, 201)

    response_data = response.json()
    assert "_id" in response_data, "Response missing '_id' key"
    assert response_data["name"] == "e2e-test-review"
...
def test_get_reviews_endpoint():
    """Test GET /api/review endpoint."""
    token = get_auth_token()
    assert token is not None, "Failed to get auth token"

    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/review", headers=headers)
...
def test_get_reviews_with_name_filter():
    ...
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/review?name=e2e", headers=headers)
    assert response.status_code == 200, _err(response, 200)
...
def test_review_endpoints_require_auth():
    ...
    response = requests.get(f"{BASE_URL}/api/review")
